...
void Radiation_ABM::run()
{
    ...
    //Check if any sites have agents
    for(unsigned home_site = 0; home_site < m_population_list_size; home_site++)
    {
      if(m_agent_list[home_site] > 0)
      {
          //Draw to find z for site with agent (local_z)
          //TODO Pick something other than a normal distribution
          //This is IMPORTANT because the derivation depends on the
          //distribution being bounded by 0 and some maxmimum
          //std::cout<<"Home Site: "<<home_site<<"\n";
          local_z = max_lognormal_value(mu, sigma, m_population_list[home_site]);


          //Order other sites by distance - already done
          //(Have to shuffle equidistant sites)
          shuffle_equidistant_sites(home_site);
         //std::cout<<"Local value: "<<local_z<<"\n";

          unsigned target_site = home_site; //initialising target_site


              //Draw for nearest site, check if z there (target z) >= local_z
              do{
              ...
              //Either way, agent leaves site (if they don't go somewhere, they disappear)
              m_agent_list[home_site]--;

              //Recheck home site to see if there are any more agents there
              home_site--;
    ...
    //TODO This is only order-agnostic because of the assumptions made;
    //      so things might get much more complicated if we change said assumptions
}
...
static PyObject* run_abm(PyObject *self, PyObject *args) {
    ...
    if (!PyArg_ParseTuple(args, "OOd", &population_list, &distance_matrix, &std_dev)) {
        // Returning NULL tells Python there's been an error.
        return NULL;
    }
    if (population_list == NULL || distance_matrix == NULL) {
        return NULL;
    }
    ...
    Radiation_ABM iterator(std_dev, population_list_ptr, population_size, distance_matrix_ptr, distance_size, flow_matrix_ptr);
...
PyDoc_STRVAR(
    module__doc__,
    "Implements a radiation model ABM in C++.\n"
);
